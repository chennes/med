...
new_coordinates_step2 = med.MEDFLOAT([12.,10., 17.,10., 22.,10.])
#  max size of profile2name : med.MED_NAME_SIZE+1
profile2name = "MIDDLE_QUAD4_PROFILE"
profile2 = med.MEDINT([8, 9, 10])
profile2size = 3

#  MED file creation
try :
	fid = med.MEDfileOpen("UsesCase_MEDmesh_6.med", med.MED_ACC_CREAT)
except RuntimeError as ex :
...
try :
	med.MEDprofileWr(fid, profile1name, profile1size, profile1)
except RuntimeError as ex :
	print("ERROR : create profile 1 ...")
	doCleanup(fid, -1)

try :
	med.MEDprofileWr(fid, profile2name, profile2size, profile2)
except RuntimeError as ex :
	print("ERROR : create profile 2 ...")
	doCleanup(fid, -1)
...
try :
	med.MEDmeshElementConnectivityWithProfileWr(fid, meshname, med.MED_NO_DT,
						med.MED_NO_IT, 0.0, med.MED_CELL, med.MED_TRIA3,
						med.MED_NODAL, med.MED_COMPACT_STMODE,
						med.MED_NO_PROFILE, med.MED_FULL_INTERLACE,
...
try :
	med.MEDmeshElementConnectivityWithProfileWr(fid, meshname, med.MED_NO_DT,
						med.MED_NO_IT, 0.0, med.MED_CELL, med.MED_QUAD4,
						med.MED_NODAL, med.MED_COMPACT_STMODE,
						med.MED_NO_PROFILE, med.MED_FULL_INTERLACE,
...
#  STEP 1 : dt1 = 5.5, it = 1
try :
	med.MEDmeshNodeCoordinateWithProfileWr(fid, meshname, 1, 1, 5.5,
					med.MED_COMPACT_STMODE, profile1name,
					med.MED_FULL_INTERLACE, med.MED_ALL_CONSTITUENT,
					nnodes, new_coordinates_step1)
except RuntimeError as ex :
	print("ERROR : nodes coordinates 1 ...")
	doCleanup(fid, -1)

#  STEP 2 : dt2 = 8.9, it = 1
try :
	med.MEDmeshNodeCoordinateWithProfileWr(fid, meshname, 2, 1, 8.9,
					med.MED_COMPACT_STMODE, profile2name,
					med.MED_FULL_INTERLACE, med.MED_ALL_CONSTITUENT,
					nnodes, new_coordinates_step2)
except RuntimeError as ex :
	print("ERROR : nodes coordinates 2 ...")
	doCleanup(fid, -1)
