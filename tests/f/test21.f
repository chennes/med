...
C
      call mfiope(fid,'test21.med',MED_ACC_RDWR, cret)
      print *,cret
      if (cret .ne. 0 ) then
...
C
      call mprcre(fid,nom1,MED_INT,desc1,dtunit1,
     &            cret)
      print *,cret
      if (cret .ne. 0 ) then
...
C
      call mprivw(fid,nom1,MED_NO_DT,MED_NO_IT,MED_UNDEF_DT,
     &            vali1,cret)
      print *,cret
      if (cret .ne. 0 ) then
...
C
      dt = 5.5D0
      call mprivw(fid,nom1,1,MED_NO_IT,dt,vali2,cret)
      print *,cret
      if (cret .ne. 0 ) then
         print *,'Erreur ecriture valeur scalaire'
         call efexit(-1)
...
C
      call mprcre(fid,nom2,MED_FLOAT64,desc2,dtunit1,
     &            cret)
      print *,cret
      if (cret .ne. 0 ) then
...
C
      call mprrvw(fid,nom2,1,2,dt,valr1,cret)
      print *,cret
      if (cret .ne. 0 ) then
         print *,'Erreur ecriture valeur scalaire'
         call efexit(-1)
