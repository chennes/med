...
C     ** ouverture du fichier                            **
 	call mfivop(fid,'test10f.med', MED_ACC_RDWR,
...
C     ** creation du champ entier n°2                      **
	call mfdcre(fid,nomcha2,MED_INT,ncomp2,comp2,unit2,
     &              dtunit1,maa1,ret)
	print *,ret
        if (ret .ne. 0 ) then
...
C     ** creation du champ entier n°3                      **
	call mfdcre(fid,nomcha3,MED_INT,ncomp3,comp3,unit3,
     &              dtunit1,maa1,ret)
	print *,ret
        if (ret .ne. 0 ) then
