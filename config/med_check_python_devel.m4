...
# PYTHON_SITE_PKG
  ...
  # echo "<---- python_arg_version_not_found : $python_arg_version_not_found --->"
  test x"$python_arg_version_not_found" == "xTrue" &&  AC_MSG_ERROR([Python interface has been enabled, then we need python devel package with C development libraries. Neither the minimal version : >= 'M4MED_PYTHON2_VERSION_REF' nor a version >= 'M4MED_PYTHON_VERSION_REF' has been found !])
