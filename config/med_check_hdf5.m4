...
## or by the --with-hdf5 option
 AC_ARG_WITH([hdf5],
             AS_HELP_STRING([--with-hdf5=<path>],[Use <path> for HDF5 path.]),
	    [],
[with_hdf5=yes])

 AC_ARG_WITH([hdf5-include],
             AS_HELP_STRING([--with-hdf5-include=<include path>],[Use <include path> for HDF5 include path.]),
	    [],
[with_hdf5_include=no])

 AC_ARG_WITH([hdf5-lib],
             AS_HELP_STRING([--with-hdf5-lib=<lib path>],[Use <lib path> for lib HDF5 path.]),
	    [],
[with_hdf5_lib=no])

 AC_ARG_WITH([hdf5-bin],
             AS_HELP_STRING([--with-hdf5-bin=<bin path>],[Use <bin path> for bin HDF5 path.]),
