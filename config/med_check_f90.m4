...
## or by the --with-f90 option
 AC_ARG_WITH([f90],
             AS_HELP_STRING([--with-f90=<compiler>],[Use <compiler> for F90 compiler (default:no)]),
 	    [
            ], [with_f90=no
 ])
])
...
#La variable f90prog mémorise le choix de compilateur F90 fait par l'utilisateur, mais les variables MPIFC et FC restent prioritaires
...
#Si l'utilisateur a explicitement demandé de ne pas contruire l'interface Fortran par l'option --enable-fortran=no, il ne faut pas lancer la détection du compilateur
...
#Test la capacité du compilateur F90 à gérer le pré-processing
