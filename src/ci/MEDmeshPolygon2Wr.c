...
/**\ingroup MEDmesh
  ...
  \details \MEDmeshPolygon2WrDetails
  \remarks
  \MEDmeshPolygon2Rem
...
/*   ISCRUTE(polyindex[indexsize-1] - polyindex[0]); */
  ...
  switch(polytype)
    ...
    case MED_POLYGON:
      break;
    case MED_POLYGON2:
      /*_multi est devenu inutile.*/
      _multi=1;
      break;
    default:
      MED_ERR_(_ret,MED_ERR_RANGE,MED_ERR_GEOMETRY,MED_ERR_VALUE_MSG);
  ...
  if (_MEDmeshAdvancedWr(fid,
			 ...
			 MED_ALL_CONSTITUENT,
			 NULL,
			 _multi*(polyindex[indexsize-1] - polyindex[0]),
			 connectivity) < 0 ) {
