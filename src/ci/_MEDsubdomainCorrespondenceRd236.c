...
void _MEDsubdomainCorrespondenceRd236(int dummy, ...) {
  ...
  if ( (numdt != MED_NO_DT) || (numit != MED_NO_IT) ) {
    ...
    SSCRUTE(jointname);SSCRUTE(localmeshname);ISCRUTE_int(localgeotype);ISCRUTE_int(localentitype);
    ISCRUTE_int(remotegeotype);ISCRUTE_int(remoteentitype);goto ERROR;
  }

  if ( MEDjointLire(fid, (char *) localmeshname,(char *)  jointname, correspondence,-1,
    ...
    MED_ERR_(_ret,MED_ERR_CALL,MED_ERR_API,"MEDjointLire");
    SSCRUTE(jointname);SSCRUTE(localmeshname);ISCRUTE_int(localgeotype);ISCRUTE_int(localentitype);
    ISCRUTE_int(remotegeotype);ISCRUTE_int(remoteentitype);goto ERROR;
  }
