...
void  _MEDfieldValueAdvancedWr41(int dummy,...) {
  ...
  /* Vérifie que l'on écrit pas dans un fichier 
     de version > à celle de notre bibliothèque */
...
/*   (*_filter).profilearraysize        = profilearraysize; */
  ...
  /*
   * Si le DataGroup MED_FIELD_GRP de niveau 0 n'existe pas => erreur
   */
  ...
  /*
   * Creation/Ouverture du datagroup de niveau 1 <numdt>.<numit>
   */
  ...
  if ( strlen((*_filter).profilename) == 0 ) {  /* idem MED_NOPFL*/
    /*Ecriture de MED_NO_PROFILE_INTERNAL car un nom de datagroup 
      ne peut pas être une chaîne vide (de nom "") */
    /*WRONG:  strncpy(_profilename,MED_NO_PROFILE,MED_NAME_SIZE+1); */
...
/*     _profilparameterexist=MED_TRUE; */
  ...
  */
  /*Le type d'entité MED_NODE_ELEMENT  */
...
REM2:
  ...
  */  
  ...
  if ( (_ret=_MEDfieldValueUpdateEntityList(fieldname,_datagroupname1,_gid,_datagroup1,entitytype,geotype  ,1)) < 0 ) {
    goto ERROR;
  }
  /* Mise à jour de la liste des types géométriques pour le type d'entité <entitype> au niveau _gid et _datagroup1 */
  if ( (_ret=_MEDfieldValueUpdateEntityList(fieldname,_datagroupname1,_gid,_datagroup1,entitytype,geotype  ,0)) <0 ) {
    goto ERROR;
  }
