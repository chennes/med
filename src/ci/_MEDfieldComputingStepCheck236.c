 ...
 *
 * - npcst : indique le nombre d'étapes de calcul utilisés par le champ <fieldname>
 *
 * ERREUR :
 * Une erreur est généré systématiquement si le nombre d'étapes de calcul n'est pas le même
				       ...
				       med_bool checkmeshname, med_bool * const samedefaultmeshname)
{
   ...
   * on vérifie qu'il y a le même nombre d'étapes de calcul (sinon le fichier n'est pas conforme au modèle 2.3.6)
   * Ceci est une vérification partielle que le modèle 2.3.6 a été respecté :
   *  - Il faudrait normalement vérifier qu'il s'agit aussi des mêmes étapes de calcul
...
/*     ISCRUTE(_nseq); */
    ...
    /*
     * On vérifie qu'il y a le même nombre d'étapes de calcul
    ...
    /*
     * On compte le nombre d'étapes de calcul pour le couple (entitytype,geotype) n°_i
     */
