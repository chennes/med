...
/**\ingroup MEDmesh
  \brief \MEDmeshGlobalNumberWrBrief
  \param fid \fid
  ...
  \details \MEDmeshGlobalNumberWrDetails
   \par Remarques
   \MEDjointDef
 */
			      ...
			      const med_int * const       number)
{
  ...
  _ret= _MEDmeshAdvancedWr(fid,
			   ...
			   numdt,
			   numit,
			   MED_UNDEF_DT, /*Si l'étape de calcul n'existe pas, ce champ sera initialisé correctement par d'autres appels.*/
			   _entitytype,
			   geotype,
			   MED_NODAL,
			   MED_UNDEF_STMODE,
			   MED_SAME_PROFILE_INTERNAL,
			   MED_NO_INTERLACE,  /*Pas d'utilité ici, car nconstituent==1*/
			   MED_ALL_CONSTITUENT,
			   NULL,
			   nentity,
			   number);
