...
void
_MEDfield23nValue236(int dummy, ...)
{
  ...
  /*
   * Cette appel n'effectue pas l'appel à _MEDfieldComputingStepCheck236
   */
  _MEDfield23nProfile236(_dummy,fid,fieldname,numdt,numit,entitytype,geotype,
			 _meshit,meshname,_tmpprofilename,_localizationname,&_nprofile );
  ...
  if ( (_n = MEDnVal(fid, (char *) fieldname,entitytype,geotype,
		     numdt,numit,(char *) meshname,(med_mode_profil) storagemode) ) < 0 ) {
