...
if (MEDcheckVersion(fid) < 0) {*fret=-1;return;}
   ...
   */
/*   if (_MEDcstring(maa,maaNoBlc) <0) { */
/*     MESSAGE("Erreur à la convertion du nom du maillage : "); */
/*     SSCRUTE(maaNoBlc); */
/*     goto ERROR; */
/*    } */
  ...
  switch(chtype)
    ...
    case MED_FLOAT64 :
      if ( _MEDdatasetNumEcrire(datagroup3,MED_NOM_CO,MED_FLOAT64,interlace,
				ncomp,numco,psize,pflmod,0,pfltab,ngauss,dimd,val) < 0) {
	MESSAGE("Impossible d'ecrire le dataset : ");
	SSCRUTE(MED_NOM_CO);ISCRUTE_size(dimd[0]); goto ERROR;
      }
      break;
