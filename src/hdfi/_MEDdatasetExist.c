  ...
  des datasets*/
{
  ...
  if ( H5Lget_info( gid, datasetname,  &_linkinfo, H5P_DEFAULT ) >= 0 ) {
    ...
    case H5L_TYPE_ERROR:
    default:
      MED_ERR_(_ret,MED_ERR_UNRECOGNIZED,MED_ERR_HDFTYPE, datasetname);
      ISCRUTE_int( _linkinfo.type);
      goto ERROR;
      break;
    }

    /*TODO : A vérifier sur un lien de dataset, à mon avis ne fonctionne pas */
    switch ( _oinfo.type ) {
    case H5G_DATASET:
    case H5G_LINK:
