...
/*     } */
    ...
    for (_cstpit=1; _cstpit <= nstep; ++_cstpit) {
      ...
      EXIT_IF(ret < 0,
	      "lors de la lecture des valeurs d'étape de calcul d'une equivalence",
	      NULL);
...
/*   med_int geo_ent_local,geo_ent_distant; */
 ...
 if ( (njnt != 0)  ) {
  ...
  /* lecture de tous les joints associes a nommaa */
  for (i = 0;i<njnt;i++) {
    ...
    for (csit=1; csit <= njstep; ++csit) {
      ...
      EXIT_IF(ret < 0,"Erreur a la lecture des valeurs (numdt,numit) dans les joints",
	    NULL);
      if ( (_numdt != MED_NO_DT) || (_numit != MED_NO_IT) ) {
	printf("Etape de calcul (numdt,numit) : ("IFORMAT","IFORMAT")\n",_numdt,_numit);
      }
      corit=1;
      while ( corit <= ncor ) {
...
/* 	  fprintf(stdout,"  %d \n",nent); */
	  ...
	  if ( (ret=MEDsubdomainCorrespondenceRd(fid,nommaa,jn,_numdt,_numit,
	    ...
	    if (!structure) {
	      for (k=0;k<nent;k++)
		fprintf(stdout,"\t\t- Correspondance "IFORMAT" : "IFORMAT" et "IFORMAT" \n",k+1,
					 ...
					 const med_connectivity_mode typ_con)
{
  ...
  EXIT_IF( (( polytype != MED_POLYGON) &&
	   ...
	   MED_ERR_RANGE_MSG, MED_ERR_GEOMETRY_MSG);
...
med_err getFieldsOn(const med_idt fid,
  ...
  med_geometry_type * _lgeotype=MED_NULL;
  med_int         _ngeo=0;
  med_int         _usedgeobyncs=0;
  char * *        AFF_lgeotype = NULL;
  ...
  */
  ...
  _ngeo=MEDfieldnGeometryType(fid,nomcha,MED_ALL_DT,MED_ALL_IT,entite);
 /* ISCRUTE(_ngeo); */
  _lgeotype=calloc(_ngeo+1,sizeof(med_geometry_type));_lgeotype[0]=MED_NO_GEOTYPE;
  MEDfieldGeometryType(fid,nomcha,MED_ALL_DT,MED_ALL_IT,entite,&_lgeotype[1],&_usedgeobyncs);
  type_geo = _lgeotype;
  AFF_lgeotype = ( char * *) calloc(_ngeo+1,sizeof(char * ));AFF_lgeotype[0]=MED_NULL;
  for(i=1;i<=_ngeo;++i) {
    AFF_lgeotype[i]=calloc(MED_NAME_SIZE+1,sizeof(char));
    MEDmeshGeotypeName(fid,_lgeotype[i],AFF_lgeotype[i]);
  }
  AFF = (const char * *) AFF_lgeotype;
...
/*   fprintf(stdout,"(********************************************************************************)\n\n"); */
  ...
  for(i=1;i<=_ngeo;++i) free(AFF_lgeotype[i]);
  free(AFF_lgeotype);
  free(_lgeotype);
  free(AFF_STRUCT);
  return ret;
		       ...
		       const int lecture_en_tete_seulement)
{
  ...
  med_int  ncstp=0,_nentity=0;
  med_entity_type * _lentitytype =NULL;
  ...
  ****************************************************************************/
  ...
  for (i =0;i<ncha;i++) {
    ...
    _nentity = MEDfieldnEntityType(fid,nomcha,MED_ALL_DT,MED_ALL_IT);
    /*ISCRUTE(MEDfieldnEntityType(fid,nomcha,MED_ALL_DT,MED_ALL_IT)); */
    _lentitytype = calloc(_nentity,sizeof(med_entity_type));
    MEDfieldEntityType(fid,nomcha,MED_ALL_DT,MED_ALL_IT,_lentitytype,&_usedbyncs);
    /* ISCRUTE(__usedbyncs); */
    for (j=0; j < _nentity ; j++) {
      if  (lret == 0)
	lret = getFieldsOn(fid, maillage, mnumdt, mnumit, nmodels,geotype_elst,geotypename_elst,
			   nomcha, dtunit, typcha, ncomp, comp, unit,_lentitytype[j],mode_coo, ncstp);
      if  (lret != 0) {
	MESSAGE("Erreur a la lecture des champs sur entités : ");SSCRUTE(MEDgetEntityTypeName(_lentitytype[j]));
	ret = -1;
      };
    }
    free(_lentitytype);
